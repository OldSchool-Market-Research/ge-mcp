...
func NewComboQuoteTool() mcp.Tool {
	return mcp.NewTool("combo_quote",
		mcp.WithDescription("Price ONE relation end-to-end at the latest quotes (archetype C's falsification primitive) — to rank the whole C universe, call combo_screen once instead of this tool per relation; combo_quote is the deep-dive for the top few (per-leg detail, reverse direction). Buy legs at `low`, sell legs at `high` minus GE tax (2%, capped 5M, per unit). meta.summary carries input_cost, output_revenue_post_tax, combo_margin (per conversion), roi_pct, min-leg volume, and units_bound = the conversions/4h the tightest buy-limit leg allows. Freshness is CADENCE-RELATIVE: each leg carries trades_24h and typical_gap_s (86400/trades), and the summary's worst_leg_age_ratio = age_s/typical_gap_s for the slowest leg — a ratio <= ~3 is that leg's normal cadence, NOT staleness; a high ratio means re-call this tool before judging, never dismiss on age alone. A leg with no traded side has price null and combo_margin is null — that leg is untradeable right now, not free. Get relation_ids from list_relations; direction=reverse only for reversible relations."),
